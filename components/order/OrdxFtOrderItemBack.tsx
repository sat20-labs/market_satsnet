...
export const OrdxFtOrderItem = ({
  ...
  const asset = useMemo(
    () =>
      item?.assets?.[0], // 假设只取第一个资产
    [item?.assets],
  );

  return (
    <div className="grid grid-cols-7 items-center text-sm text-gray-200 border-b border-zinc-800 px-4 py-2 hover:bg-zinc-800">
      {/* Name */}
      ...
      <div className="col-span-1 text-right">
        ...
        <span className="font-mono text-gray-100">&nbsp;sats</span>
      </div>
      ...
      <div className="col-span-1 text-right">
        <span className="font-mono text-gray-100">
          $
          <BtcPrice
            btc={asset?.unit_price / 100000000}
          />
        </span>
      </div>
      ...
      <div className="col-span-1 text-right">
        <WalletConnectBus className="w-full">
          {item?.address === currentAddress && showResale ? (
            <Button
              className="text-tiny h-8 w-full bg-gradient-to-r from-indigo-500/50 via-purple-500/50 to-pink-500/50 hover:border-none hover:from-indigo-500 hover:via-purple-500 hover:to-pink-500"
              variant="flat"
              radius="sm"
              startContent={
                item.locked == '1' ? (
            ...
            <Button
              className="h-8 w-full bg-gradient-to-r from-indigo-500/50 via-purple-500/50 to-pink-500/50 hover:border-none hover:from-indigo-500 hover:via-purple-500 hover:to-pink-500"
              variant="ghost"
              size="md"
              isDisabled={!canBuy}
              isLoading={loading}
    ...
    <div>
      {mockData.map((item, index) => (
        <OrdxFtOrderItem
          key={index}
          item={item}
          onBuy={() => console.log('Buying:', item)}
          onCancelOrder={() => console.log('Canceling order:', item)}
        />
      ))}
    </div>
  );

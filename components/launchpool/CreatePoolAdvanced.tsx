...
export default function CreatePoolAdvanced({ closeModal }: Props) {
    ...
    const handleLargePoolConfirm = async () => {
        await deployContract();
    };

    const handleLargePoolBack = () => {
        setShowLargePoolWarning(false);
    };
    ...
    async function handleConfirm(event: React.MouseEvent<HTMLButtonElement, MouseEvent>): Promise<void> {
        ...
        // 检查池子资金量是否超过 100 BTC
        const poolFundsBtc = Number(estimatedPoolFunds);
        if (poolFundsBtc > 100) {
            setShowLargePoolWarning(true);
            return;
        }
    ...
    const estimatedPoolFunds = useMemo(() => {
        ...
        const launchRatio = Number(formData.launchRatio) / 100;
        const mintAmtPerSat = Number(formData.mintAmtPerSat);

        if (maxSupply > 0 && launchRatio > 0 && mintAmtPerSat > 0) {
            const sats = Math.floor((maxSupply * launchRatio) / mintAmtPerSat);
            const btc = sats / 1e8;
    ...
    return (
        <>
            {/* 大池子警告模态框 */}
            {showLargePoolWarning && (
                <Modal onClose={() => setShowLargePoolWarning(false)}>
                    <div className="p-6">
                        <div className="mb-4">
                            <h3 className="text-lg font-bold mb-4 text-red-500">
                                {t('pages.createPool.largePoolWarning.title')}
                            </h3>
                            <p className="text-red-500 font-semibold mb-2">
                                {t('pages.createPool.largePoolWarning.message')}
                            </p>
                            <p className="text-zinc-400 text-sm">
                                {t('pages.createPool.largePoolWarning.description')}
                            </p>
                        </div>
                        <div className="flex justify-end gap-3">
                            <Button variant="outline" onClick={handleLargePoolBack} className="px-4 py-2">
                                {t('pages.createPool.largePoolWarning.back')}
                            </Button>
                            <Button className="btn-gradient px-4 py-2" onClick={handleLargePoolConfirm}>
                                {t('pages.createPool.largePoolWarning.confirm')}
                            </Button>
                        </div>
                    </div>
                </Modal>
                ...
                <div className="p-6 max-w-[1360px] mx-auto bg-zinc-800/50 border border-zinc-800 rounded-lg shadow-lg">
                    {step === 1 && (
                        <div className="mt-4">
                            <div className="text-lg font-bold h-10">
                                <span className="px-4 py-2 border-l-6 border-purple-500">
                                    {t('pages.createPool.step1.title')}
                                </span>
                            </div>
                            ...
                            <div className="flex justify-items-start items-center mt-4 gap-4">
                                <label className="block text-sm font-medium text-gray-300">
                                    {t('pages.createPool.protocol.title')}
                                </label>
                                <Select onValueChange={(value) => handleInputChange('protocol', value)}>
                                    <SelectTrigger className="w-56 py-4 h-12">
                                        {t(`pages.createPool.protocol.${formData.protocol}`) ||
                                            t('pages.createPool.selectProtocol')}
                                    </SelectTrigger>
                                    <SelectContent className="max-h-60 overflow-y-auto">
                                        <SelectItem value="ordx" className="h-9 py-2">
                                            {t('pages.createPool.protocol.ordx')}
                                        </SelectItem>
                            ...
                            {formData.protocol === 'runes' && (
                                <p className="mt-1 text-xs text-gray-400">{t('pages.createPool.runesTickerNote')}</p>
                            )}

                            <label className="block text-sm font-medium text-gray-300 mt-4 mb-1">
                                {t('pages.createPool.maxSupply')}
                            </label>
                            <Input
                            ...
                            {formData.protocol === 'ordx' && (
                                <>
                                    ...
                                    <Input
                                        ...
                                        disabled={false}
                                    />
                                    {errorMessage && <p className="mt-1 text-xs text-red-400 gap-2">* {errorMessage}</p>}
                                </>
                            )}
                            ...
                            {formData.protocol === 'runes' && (
                                <>
                                    <label className="block text-sm font-medium text-gray-300 mt-4 mb-1">
                                        {t('pages.createPool.assetSymbol')}
                                    </label>
                                    <Input
                    ...
                    {step === 2 && (
                        <div className="mt-4">
                            <div className="text-base font-bold h-10">
                                <span className="px-4 py-2 border-l-6 border-purple-500">
                                    {t('pages.createPool.step2.title')}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center mt-4">
                                <label className="block text-sm font-medium text-gray-300 mb-1">
                                    {t('pages.createPool.mintAmtPerSat')}
                                </label>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <span className="ml-1 cursor-pointer align-middle inline-flex">
                            ...
                            <label className="block text-sm font-medium mt-4 text-gray-300 mb-1">
                                {t('pages.createPool.limit')}
                            </label>
                            <Input
                            ...
                            <label className="block text-sm font-medium text-gray-300 mt-4 mb-1">
                                {t('pages.createPool.launchRatio')}
                            </label>
                            <Input
                            ...
                            <p className="mt-1 text-xs text-gray-400">
                                {t('pages.createPool.step2.estimatedPoolFunds')} :
                    ...
                    {step === 3 && (
                        <div className="mt-4">
                            <div className="text-base font-bold h-10">
                                <span className="px-4 py-2 border-l-6 border-purple-500">
                                    {t('pages.createPool.step3.title')}
                                </span>
                            </div>
                            <p className="text-sm text-zinc-400 mt-2">
                                {t('Monitor the progress of your launch pool and user participation.')}
                            </p>
                            ...
                            {contractURL && (
                                <div className="mt-6 p-4 bg-zinc-900 rounded-lg border border-zinc-60">
                                    <div className="mb-2 font-bold text-white">
                                        {t('pages.createPool.step3.poolStatus')}
                                    </div>
                                    <div className="text-sm text-zinc-300 mb-2">
                                        {t('pages.createPool.step3.status')}:{' '}
                                        {statusTextMap[String(poolStatusData?.status)] ??
                                            poolStatusData?.status ??
                                            '-'}
                                    </div>

                                    <div className="text-sm text-zinc-300 mb-2">
                                        DeployTickerTxId:{' '}
                                        {poolStatusData?.DeployTickerTxId ? (
                                            <a
                                            ...
                                            >
                                                {hideStr(poolStatusData.DeployTickerTxId, 6)}
                                            </a>
                                        ) : (
                                            '-'
                                        )}
                                    ...
                                    <div className="text-sm text-zinc-300 mb-2">
                                        ...
                                        {poolStatusData?.MintTxId ? (
                                            <a
                                                ...
                                                className="text-blue-400 underline"
                                            >
                                                {hideStr(poolStatusData.MintTxId, 6)}
                                            </a>
                                        ) : (
                                            '-'
                                        )}
                                    ...
                                    <div className="text-sm text-zinc-300 mb-2">
                                        ...
                                        {poolStatusData?.AnchorTxId ? (
                                            <a
                                                ...
                                                className="text-blue-400 underline"
                                            >
                                                {hideStr(poolStatusData.AnchorTxId, 6)}
                                            </a>
                                        ) : (
                                            '-'
                                        )}
                            ...
                            <p className="text-red-500 text-xs font-bold mt-3">
                                {t('pages.createPool.step3.note')}:&nbsp;
                                <span className="ml-1 text-zinc-300">
                                    {t('pages.createPool.step3.noteDescription')}
                                </span>
                            </p>
                        </div>
                    )}
                    ...
                    <div className="mt-4 py-4 flex justify-between">
                        {step > 1 && step < 3 && (
                            <Button className="w-40 sm:w-48" variant="outline" onClick={handlePrevStep}>
                                {t('pages.createPool.previous')}
                            </Button>
                        )}
                        {step === 1 && (
                            <Button

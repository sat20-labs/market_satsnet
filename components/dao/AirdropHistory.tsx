...
export function AirdropHistory({ contractUrl }: { contractUrl: string }) {
  ...
  const [selectedOrderType, setSelectedOrderType] = useState<number | 'all'>(
    'all',
  );
  const [detailModalOpen, setDetailModalOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<ContractHistoryItem | null>(
    null,
  );
  ...
  const formatAmount = (
    amt: number | { Precision: number; Value: string } | null,
  ): string => {
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-3">
        <h3 className="text-lg font-semibold text-white">
          {t('pages.dao.workflow.airdrop_history.history_list', {
            defaultValue: '合约历史记录',
          })}
        </h3>
        <div className="flex items-center gap-3">
          <div className="text-sm text-zinc-400">
            {total > 0
              ? t('pages.dao.workflow.airdrop_history.total_records', { total })
              : t('pages.dao.workflow.airdrop_history.no_records')}
          </div>
          <Select
            value={String(selectedOrderType)}
            onValueChange={(value) => {
              setSelectedOrderType(value === 'all' ? 'all' : Number(value));
              setCurrentPage(1);
            }}
          >
            <SelectTrigger className="w-[180px] bg-zinc-800 border-zinc-700 text-white">
              <SelectValue
                placeholder={t(
                  'pages.dao.workflow.airdrop_history.filter_placeholder',
                )}
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">
                {t('pages.dao.workflow.airdrop_history.filter_all')}
              </SelectItem>
      ...
      {filteredHistory.length === 0 ? (
        <div className="text-center text-zinc-500 py-8">
          {t('pages.dao.workflow.no_airdrop_history', {
            defaultValue: '暂无记录',
          })}
        </div>
      ) : (
              ...
              <TableHeader>
                <TableRow className="border-zinc-800">
                  ...
                  <TableHead className="text-zinc-400">
                    {t('pages.dao.workflow.airdrop_history.order_type')}
                  </TableHead>
                  <TableHead className="text-zinc-400">
                    {t('pages.dao.workflow.airdrop_history.address')}
                  </TableHead>
                  <TableHead className="text-zinc-400">
                    {t('pages.dao.workflow.airdrop_history.amount')}
                  </TableHead>
                  <TableHead className="text-zinc-400">
                    {t('pages.dao.workflow.airdrop_history.time')}
                  </TableHead>
                  <TableHead className="text-zinc-400">
                    {t('pages.dao.workflow.airdrop_history.txid')}
                  </TableHead>
                  <TableHead className="text-zinc-400">
                    {t('pages.dao.workflow.airdrop_history.action')}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {paginatedHistory.map((item, index) => {
                  const hasDetails =
                    (item.airdrop?.items && item.airdrop.items.length > 0) ||
                    item.validate ||
                    (item.bind?.items && item.bind.items.length > 0) ||
                    item.register;
                  return (
                      ...
                      <TableRow className="border-zinc-800">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {(item.OutTxId || item.InUtxo) && (
                              <Button
                                ...
                                className="h-4 w-4 p-0"
                                onClick={() =>
                                  copyToClipboard(item.OutTxId || item.InUtxo)
                                }
                              >
                                <Copy className="h-3 w-3 ml-3 text-gray-400" />
                              </Button>
                        ...
                        <TableCell>
                          {hasDetails && (
                            ...
                            >
                              {t(
                                'pages.dao.workflow.airdrop_history.view_details',
                              )}
                            </Button>
                          )}
                        </TableCell>
                ...
                <div className="my-4">
                  <div className="text-[16px] font-medium text-white mb-4">
                    <span className="text-zinc-400">
                      <span className="pb-2 text-[18px] text-purple-500">
                        [{getOrderTypeLabel(selectedItem.OrderType)}]
                      </span>
                      &nbsp;
                    ...
                    <TableHeader>
                      <TableRow className="border-zinc-700">
                        {selectedItem.OrderType === 18 ? (
                          <>
                            <TableHead className="text-zinc-400 bg-zinc-800">
                              {t('pages.dao.workflow.airdrop_history.uid')}
                            </TableHead>
                            <TableHead className="text-zinc-400 bg-zinc-800">
                              {t('pages.dao.workflow.airdrop_history.status')}
                            </TableHead>
                            <TableHead className="text-zinc-400 bg-zinc-800">
                              {t('pages.dao.workflow.airdrop_history.reason')}
                            </TableHead>
                          </>
                        ) : selectedItem.OrderType === 15 ? (
                          <>
                            <TableHead className="text-zinc-400 bg-zinc-800">
                              UID
                            </TableHead>
                          ...
                          <>
                            ...
                            <TableHead className="text-zinc-400">
                              {t('pages.dao.workflow.airdrop_history.result')}
                            </TableHead>
                          </>
                        )}
                      </TableRow>
                    ...
                    <TableBody>
                      ...
                      selectedItem.validate ? (
                        <TableRow className="border-zinc-700">
                          ...
                          <TableCell className="font-mono text-white">
                            <div className="space-y-2">
                              <div>
                                {selectedItem.Done === 1
                                  ? selectedItem.validate.reason === 'validated'
                                    ? '✅ Approved'
                                    : '❌ Rejected'
                                  : '⏳ Processing'}
                              </div>
                          ...
                          <TableCell className="font-mono text-white">
                            <div className="space-y-2">
                              <div className="py-2">
                                <span className="bg-purple-800 text-zinc-200 rounded">
                                  <span className="p-2">
                                    {getOrderTypeLabel(
                                      selectedItem.validate?.type,
                                    )}
                                  </span>
                                </span>{' '}
                                : {selectedItem.validate.reason}
                              </div>
                            </div>
                          </TableCell>
                        </TableRow>
                        ...
                        <TableRow className="border-zinc-700">
                          <TableCell className="font-mono text-white">
                            <div className="space-y-2">
                              <div>{selectedItem.register.uid}</div>
                            </div>
                          </TableCell>
                          <TableCell className="font-mono text-white">
                            <div className="space-y-2">
                              <div>
                                {selectedItem.register.referrerUID || '-'}
                              </div>
                            </div>
                          </TableCell>
                          <TableCell className="font-mono text-white">
                        ...
                        selectedItem.bind?.items ? (
                        (selectedItem.airdrop?.items ||
                          selectedItem.bind?.items)!.map((detail, idx) => (

...
export default function TradeRankPanel() {
    const [period, setPeriod] = React.useState<'total' | 'month'>('total');
    const { entries, hasMore, loadMore, resetAndLoad, loading, initialized } = useRankPagination('trade', period, 20);
    ...
    const changePeriod = (p: 'total' | 'month') => {
        if (p === period) {
            // same period => refresh explicitly
            resetAndLoad();
        } else {
            // change period => let hooks reset and effect trigger load
            setPeriod(p);
        }
    };
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <div>
                    <div className="text-white font-medium">{t('pages.points.trade_rank_title')}</div>

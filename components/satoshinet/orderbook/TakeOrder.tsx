...
const TakeOrder = ({ assetInfo, mode, setMode, userWallet, orders }: TakeOrderProps) => {
  ...
  const queryKey = useMemo(() => {
    return ['orders', assetInfo.assetName, chain, network, page, size];
  }, [assetInfo.assetName, page, size, network, chain]);
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () =>
      marketApi.getOrders({
        offset: (page - 1) * size,
        size,
        assets_protocol: assetInfo.assetName.split(':')[0],

...
const LightweightKline = dynamic(
  () =>
    import("@/components/satoshinet/LightweightKline").then(
      (m) => m.LightweightKline,
    ),
  { ssr: false },
);
...
function OrderPageContent() {
  ...
  const protocol = useMemo(
    () => swapStatusData?.Contract?.assetName?.Protocol || "",
    [swapStatusData],
  );
  ...
  const hasLpt = useMemo(() => {
    ...
    const value =
      typeof lptAmt.Value === "string"
        ? parseFloat(lptAmt.Value)
        : lptAmt.Value;
    return value > 0;
  }, [lptAmt]);
    ...
    <div className="w-full" suppressHydrationWarning>
      <div className="grid grid-cols-1 md:grid-cols-12 sm:gap-6 p-2 sm:pt-4 h-full w-full">
        ...
        <div className="md:col-span-8 flex flex-col gap-3 order-last md:order-1">
          ...
          <div
            className={`relative overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 ${chartHeights_div}`}
          >
            <div className="absolute top-5 sm:top-3 right-2 z-20 flex gap-2">
              <button
                onClick={() => setChartModeAndStore("line")}
                className={`px-2 py-1 rounded text-[11px] border transition-colors ${chartMode === "line" ? "btn-gradient text-white border-purple-500" : "bg-zinc-800/70 text-zinc-400 border-zinc-700 hover:text-zinc-200"}`}
              >
                <Icon icon="lucide:chart-spline" className="w-4 h-4" />
              </button>
              <button
                onClick={() => setChartModeAndStore("lw")}
                className={`px-2 py-1 rounded text-[11px] border transition-colors ${chartMode === "lw" ? "btn-gradient text-white border-purple-500" : "bg-zinc-800/70 text-zinc-400 border-zinc-700 hover:text-zinc-200"}`}
              >
                <Icon
              ...
              <div className={`w-full ${chartHeights_div}`}>
                ...
                {hydrated && chartMode === "lw" && (
                  <div className="w-full h-full">
                    <LightweightKline
                      symbol={contractUrl}
                      initialResolution="4H"
                      chartHeights={{ volumeRatio: 0.3, height: cHeights }}
                    />
                  </div>
                )}
              </div>
          ...
          <div className="md:hidden relative z-10">
            <details className="rounded-xl border border-zinc-800 bg-zinc-900">
              <summary className="px-3 py-2 cursor-pointer text-sm text-zinc-300 select-none">
                {t("common.assetTradeInfo")}
              </summary>
              <div className="p-3">
                <AssetInfo
                  depthData={swapStatusData}
            ...
            <Tabs defaultValue="swap" className="w-full">
              ...
              <TabsContent value="swap">
                ...
                <Swap
                  ...
                  refresh={refreshAll}
                  isRefreshing={
                    isSwapStatusLoading || isAnalyticsLoading || isTickerLoading
                  }
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <ButtonRefresh
                        ...
                        loading={isSwapStatusLoading}
                        className="ml-2 bg-zinc-800/50"
                      />
                    </div>
      ...
      <div className="w-full px-2 sm:px-2 mt-4 sm:m-1 bg-transparent">
        ...
        >
          <UnderlineTabsList className="mb-2 text-xs" suppressHydrationWarning>
            <UnderlineTabsTrigger value="activities" suppressHydrationWarning>
              {t("common.activity") || "Activities"}
            </UnderlineTabsTrigger>
            <UnderlineTabsTrigger value="myactivities" suppressHydrationWarning>
              {t("common.my_activities") || "My Activities"}
            </UnderlineTabsTrigger>
            <UnderlineTabsTrigger value="holders" suppressHydrationWarning>
              {t("common.holder") || "Holders"}
            </UnderlineTabsTrigger>
            <UnderlineTabsTrigger value="lpholders" suppressHydrationWarning>
              {t("common.lptHolders") || "LP-Holders"}
            </UnderlineTabsTrigger>
          </UnderlineTabsList>
